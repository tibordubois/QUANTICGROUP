...
class qBayesNet:
    ...
    getAllParentStates(self, node: Union[str, int]) \
    ...
    def getWidth(self, node: Union[str, int]) -> int:
        """
        Calculate the number of qubits required to represent a variable in the Bayesian Network.
    
        This method determines the width of the quantum register needed for a given node. The width
        is computed based on the domain size of the variable associated with the node, using the formula:
        ceil(log2(domain_size)). This ensures that the quantum register can represent all possible states
        of the variable.
    
        Parameters:
        ----------
        node : Union[str, int]
            The name (string) or ID (integer) of the node in the Bayesian network for which to calculate
            the quantum register width.
    
        Returns:
        -------
        int
            The number of qubits required to adequately represent the variable associated with the given node.
    
        Raises:
        ------
        KeyError
            If the specified node does not exist in the Bayesian network.
    
        Example:
        --------
        >>> qbn = qBayesNet(bn)
        >>> qbn.getWidth('Weather')
    
        Notes:
        -----
        This calculation is crucial for setting up the quantum registers in the quantum circuit representation
        of the Bayesian network.
        """
    ...
    def getTotNumQBits(self) -> int: #not used
        """
        Calculate the total number of qubits required to represent the entire Bayesian Network as a quantum circuit.
    
        This method computes the cumulative number of qubits needed to represent all variables in the Bayesian network. 
        Each variable requires a number of qubits proportional to the logarithm of the domain size, rounded up to 
        the nearest whole number (as calculated by getWidth). 
        ...
        Notes:
        -----
        This method uses the formula defined in Equation 21 as referenced in 
        "Quantum circuit representation of Bayesian networks - Sima E. Borujeni".
        """
    ...
    def getAllParentStates(self, node: Union[str, int]) \
                                -> list[dict[Union[str, int]: int]]:
        """
        Gives all the possible parent state combinations of a given variable
    ...
    def getQuantumRegisters(self) -> dict[int: QuantumRegister]:
        """
        Create and return Quantum Registers for each target node in the Bayesian Network.
        ...
        dict[int, QuantumRegister]
            A dictionary mapping each node's ID within the target nodes of the Bayesian network to 
            its corresponding Quantum Register.
            Each register's label corresponds to the node ID.
    
        Example:
        --------
        >>> qbn = qBayesNet(bn)
        >>> qbn.getQuantumRegisters()
        {0: QuantumRegister(2, '0'), 1: QuantumRegister(3, '1')}
        # node 0 requires a Quantum Register of size 2, and node 1 requires size 3.
        """
        res = dict()
    ...
    def indicatorFunction(self, binary_list: list[list[int]],
        ...
        """
        Gives a list of matches when given a list of binary strings represented using lists and a dictionary 
        of conditions based on the equations 17 and 19 of the article : 
        "Quantum circuit representation of Bayesian networks - Sima E. Borujeni".
        ...
        binary_list: list[list[int]]
            ...
            the superposition state representing a variable in the context of the equation 16 
            referencing the basis article previously mentioned.
    ...
    def getProbability(self, value: int, 
        ...
        representing the variable and other nodes in the Bayesian Network referencing two equations 
        from the basis article, 18 and 20. 
    ...
    def multiQubitRotation(self, circuit: QuantumCircuit, 
        ...
        This method implements a series of controlled rotations on a set of qubits representing a Bayesian network variable.
        The rotations are calculated to encode the conditional probabilities of the variable's states into the amplitudes 
        of its corresponding quantum states. The method uses recursive application of controlled rotations to effectively
        build the part of the circuit that represents the probabilistic dependencies defined in the network. The implementation
        follows the approach described in Figure 9 and Equation 18 of the article "Quantum circuit representation of Bayesian 
        networks - Sima E. Borujeni".
    ...
    def buildCircuit(self, add_measure: bool = True,
        ...
        for n_id in internal_nodes.intersection(self.target_nodes):
            ...
            for params_dict in self.getAllParentStates(n_id): #params is dict
